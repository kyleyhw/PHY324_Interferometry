...
def plot_residuals(show=True, save=False):
        ...
        fit2 = fitting.Fitting(model=model, x=data.x, x_error=data.x_error, y_measured=data.full_data,
                              y_error=data.errors, units_for_parameters=('', ''))

        fig2, ax2 = plt.subplots(1, 1, figsize=(16, 9))

        fit2.plot_residuals(ax2)
        ...
        if show:
                fig2.show()
        if save:
                fig2.savefig('PHY324_Interferometry_calibration_curve_residuals.png')


fit_data_to_fringes(show=False, save=True)
fit_data_to_path_length(show=False, save=True)
plot_residuals(show=False, save=True)